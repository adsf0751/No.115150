...
extern	int ginToggleCardRetention;
...
int SALE_TRT_NCCC_ATS_TABLE[] =
{
	...
	_NCCC_FUNCTION_CHESG_CHECK_, /* [114197] 中心收單作業ESG永續發展優化 輸入金額後 詢問持卡人是否同意接收數位化簽帳單  */
	...
	_NCCC_FUNCTION_DISPLAY_CHESG_, /* [114197] 中心收單作業ESG永續發展優化 螢幕顯示數位化簽帳單 */
...
int inNCCC_Func_BuildAndSendPacket_Demo_Flow(TRANSACTION_OBJECT* pobTran)
{
	 ...
	 * [20260520] 新增 ESG Demo 的訊息,因需選擇才會使用,所以不分交易類型都進行複制
	 */
	memset(pobTran->szCHESGGuid, 0x00, sizeof(pobTran->szCHESGGuid));
	strncat(pobTran->szCHESGGuid, "教育訓練版不產生數位化簽帳單網址URL", sizeof(pobTran->szCHESGGuid) - 1);
...
int inNCCC_Func_Check_Special_Card_In_OPT_Flow(TRANSACTION_OBJECT* pobTran)
{
	...
	/* [114197] 中心收單作業ESG永續發展優化 
	 * [20260623] 因 _NCCC_FUNCTION_CHESG_CHECK_ 的步驟調整到刷卡前
	 * 所以會影響輸入金額後的交易流程,會無法判斷是什麼交易
	 * 所以借用檢查卡片步驟,再次檢查是否能進行 ESG 功能*/
...
int inNCCC_Func_RePrint_Customer_Flow(TRANSACTION_OBJECT *pobTran)
{
	...
	char	szCustomIndicator[3 + 1] = {0};
        ...
        vdUtility_SYSFIN_LogMessage(AT, "inNCCC_Func_RePrint_Customer_Flow START!");
		...
		do
		{
			...
			if (pobDataTran.srBRec.inChipStatus != 0		|| 
			    ...
			    pobDataTran.srBRec.uszEMVFallBackBit == VS_TRUE     ||
                            pobDataTran.srBRec.uszCUPEMVQRCodeBit == VS_TRUE)
			{
...
int inNCCC_Func_Check_And_Recover_Settlement_XML(TRANSACTION_OBJECT *pobTran, char* szFileName, char* szPath)
{
	...
	snprintf(szFileName_New, sizeof(szFileName_New), "%s%s", szPath, szFileName);
